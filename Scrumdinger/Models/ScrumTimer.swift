...
final class ScrumTimer: ObservableObject {
    ...
    private(set) var lengthMinutes: Int
    ...
    private var lengthInSeconds: Int { lengthMinutes * 60 }
    private var secondsPerSpeaker: Int {
        (lengthMinutes * 60) / speakers.count
    }
    ...
    init(lengthInMinutes: Int = 0, attendees: [DailyScrum.Attendee] = []) {
        self.lengthMinutes = lengthInMinutes
    ...
    nonisolated private func update() {
        ...
        Task { @MainActor in
            ...
            secondsElapsedForSpeacker = secondsElapsed
            self.secondsElapsed = secondsPerSpeaker * speakerIndex + secondsElapsedForSpeacker
            guard secondsElapsed <= secondsPerSpeaker else {return}
            secondsRemaining = max(lengthInSeconds - self.secondsElapsed, 0)
            
            if secondsElapsedForSpeacker >= secondsPerSpeaker {
                changeToSpeaker(at: speakerIndex + 1)
    ...
    func reset(lengInMinutes: Int, attendees: [DailyScrum.Attendee]) {
        self.lengthMinutes = lengInMinutes
        self.speakers = attendees.speakers
        secondsRemaining = lengthInSeconds
        activeSpeaker = speakerText
    }
